...
def _load_eval_data(dataset, query_size, agent, smoothing_weight=0.0):
    if agent in ["Oracle", "UpperBound"]:
        file = os.path.join("runs", dataset, agent, "accuracies.csv")
    else:
        file = os.path.join("runs", dataset, str(query_size), agent, "accuracies.csv")
    all_runs = pd.read_csv(file, header=0, index_col=0)
...
def full_plot(dataset:str, query_size=None, show_auc=True, radjust=0.8, forecast_oracle=True,
    ...
    if query_size is None:
        ...
        query_size = [x for x in query_size if x.isdigit()]  # Keep only numeric values
        query_size = sorted(query_size, key=int)
...
def get_agent_by_name(name:str)->Callable:
    ...
    elif name == "clustermargin":
        return agents.ClusterMargin
    else:
...
def _create_table_for_query_size(dataset, query_size, y_label, title, smoothing_weight, forecast_oracle=True):
    inferred_x_axis = None
    rows = []
    ...
    df.sort_values(by=["Agent", "X"], inplace=True)

    # Compute per-agent means
    mean_rows = df.groupby("Agent")[y_label].mean().reset_index()
    mean_rows["Query Size"] = query_size
    # mean_rows["X"] = "Mean"
    mean_rows = mean_rows[["Agent", "Query Size", y_label]]

    df_with_mean = pd.DataFrame(mean_rows)
...
def full_table(dataset: str, query_size=None, forecast_oracle=True,
               smoothing_weight=0.0, y_label="Accuracy", table_title=None):
    ...
    if query_size is None:
        ...
        if "UpperBound" in query_size:
            query_size.remove("UpperBound")
        query_size = [x for x in query_size if x.isdigit()]
        query_size = sorted(query_size, key=int)
    ...
    if isinstance(query_size, list):
        for q in query_size:
            df = _create_table_for_query_size(dataset, q, y_label, f"{table_title} - {q}", smoothing_weight, forecast_oracle)
            dfs.append(df)
    elif isinstance(query_size, int):
        df = _create_table_for_query_size(dataset, query_size, y_label, table_title, smoothing_weight, forecast_oracle)
        dfs.append(df)
    else:
        raise ValueError()
